...
if (isset($_GET["productCode"])) {
    ...
    if ($row) {
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            $stmt = $conn->prepare($sql);
            $stmt->execute([$productName, $productLine, $productScale, $productVendor, $productDescription, $quantityInStock, $buyPrice, $sellPrice, $productCode]);
            $stmt->closeCursor();
...
<body>
    ...
    <form method="post" action="<?php echo $_SERVER["PHP_SELF"] . "?productCode=" . $productCode; ?>">
        ...
        <input type="number" id="sellPrice" name="sellPrice" step="0.01" value="<?php echo $row['sellPrice']; ?>"><br>
